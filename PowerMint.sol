...
contract PowerMint is
    ...
    Ownable
{
    constructor() ERC721(" PowerMInt", "PWM") {
    ...
    struct mintinglimitscount {
        uint256 totalLimitCount;
        uint256 whitelistLimitCount;
        uint256 PlatformLimitCount;
        uint256 publicLimitCount;
        uint256 userLimitCount;
    }
    ...
    mapping(uint256 => adminsrecord) public adminsRecord;
    mapping(address => bool) public isAdmin;
    mapping(uint256 => mintinglimits) public mintingLimits;
    mapping(uint256 => mintinglimitscount) public LimitsCount;
    ...
    event WhitlistUserMinited(
        address calledby,
        address mintedto,
        uint256 tokenId,
        string name
    );
    event PulblicUserMinited(
        address calledby,
        address mintedto,
        uint256 tokenId,
        string name
    ...
    event MintingLimitUpdated(address updatedby, uint256 limit);
    ...
    error AdminOrIdAlreadyExist(address addrs, uint256 Id);
    error WUserAlreadyExist(address addrs, uint256 Id);
    error IdNotFound(uint256 Id);
    ...
    function becomeAdmin(
        ...
        if (isAdmin[_address] == true || adminIdCheck[_setAdminId] == true)
            revert AdminOrIdAlreadyExist(_address, _setAdminId);
        ...
        if (msg.value == adminPrice) {
            ...
            if (_whitelistlimit + _platformlimit <= _totallimit) {
                ...
                mintingLimits[_setAdminId].publicLimit =
                    _totallimit -
                    (_whitelistlimit + _platformlimit);
    ...
    function setAdminPrice(uint256 _price) public onlyOwner {
        adminPrice = _price;
    ...
    function whitelistMinting(
        ...
        if (
            ...
            mintingLimits[_adminId].totalLimit
        ...
        if (
            ...
            mintingLimits[_adminId].userLimit
        )
    ...
    function publicSaleEnbDis(uint256 _adminId) public {
        if (isAdmin[msg.sender] == false) revert AdminNotFound(msg.sender);
        if (adminDetails[msg.sender].adminId != _adminId)
            revert IdNotFound(_adminId);
        if (mintingLimits[_adminId].publicSale == false) {
    ...
    function mintingEnbDis(uint256 _adminId) public {
        if (isAdmin[msg.sender] == false) revert AdminNotFound(msg.sender);
        if (adminDetails[msg.sender].adminId != _adminId)
            revert IdNotFound(_adminId);
        if (mintingLimits[_adminId].mintingStatus == false) {
    ...
    function whitelistUser(uint256 _adminId, address _address)
        public
        onlyAdmins
    {
        if (adminDetails[msg.sender].adminId != _adminId)
    ...
    function whitelistUsers(uint256 adminId)
        public
        view
